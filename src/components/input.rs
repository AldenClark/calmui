...
use gpui::{
    ...
    IntoElement, MouseButton, ParentElement, RenderOnce, SharedString, Styled, UTF16Selection,
...
type SelectionRange = Option<(usize, usize)>;
type NormalizedEdit = (String, usize, SelectionRange, SelectionRange);
...
impl TextInput {
    ...
    pub(crate) fn refine_style(mut self, style: &gpui::StyleRefinement) -> Self {
        gpui::Refineable::refine(&mut self.style, style);
        self
    }
...
impl RenderOnce for PasswordInput {
    fn render(self, window: &mut gpui::Window, cx: &mut gpui::App) -> impl IntoElement {
        let inner = self.id.ctx().root(self.inner).refine_style(&self.style);
...
impl RenderOnce for PinInput {
    fn render(mut self, window: &mut gpui::Window, cx: &mut gpui::App) -> impl IntoElement {
        ...
        let field = if self.disabled || self.read_only {
            div()
                .cursor_default()
                .child(root.with_enter_transition(self.id.slot("enter"), self.motion))
        } else {
            div()
                .cursor_text()
                .child(root.with_enter_transition(self.id.slot("enter"), self.motion))
        ...
        if let Some(error) = self.error {
            Stack::vertical()
                ...
                .child(
                    div()
                        .text_size(self.theme.components.input.error_size)
                        .text_color(resolve_hsla(&self.theme, self.theme.components.input.error))
                        .child(error),
                )
                .into_any_element()
        } else {
...
crate::impl_disableable!(TextInput, |this, value| this.disabled = value);
crate::impl_disableable!(PinInput, |this, value| this.disabled = value);
crate::impl_variant_size_radius_via_methods!(TextInput, variant, size, radius);
crate::impl_variant_size_radius_via_methods!(PinInput, variant, size, radius);

impl crate::contracts::Disableable for PasswordInput {
    fn disabled(mut self, value: bool) -> Self {
        self.inner = crate::contracts::Disableable::disabled(self.inner, value);
        self
    }
}

impl crate::contracts::Varianted for PasswordInput {
    fn with_variant(mut self, value: Variant) -> Self {
        self.inner = crate::contracts::Varianted::with_variant(self.inner, value);
        self
    }
}

impl crate::contracts::Sized for PasswordInput {
    fn with_size(mut self, value: Size) -> Self {
        self.inner = crate::contracts::Sized::with_size(self.inner, value);
        self
    }
}

impl crate::contracts::Radiused for PasswordInput {
    fn with_radius(mut self, value: Radius) -> Self {
        self.inner = crate::contracts::Radiused::with_radius(self.inner, value);
        self
    }
}
