...
#[cfg(test)]
static CONTROL_TEST_LOCK: LazyLock<Mutex<()>> = LazyLock::new(|| Mutex::new(()));
...
mod tests {
    ...
    struct ControlTestGuard {
        _lock: MutexGuard<'static, ()>,
    }

    fn guard() -> ControlTestGuard {
        let lock = lock_test_store();
        clear_all();
        ControlTestGuard { _lock: lock }
    }

    impl Drop for ControlTestGuard {
        fn drop(&mut self) {
            clear_all();
        }
    }
