...
use gpui::{
    AnyElement, Bounds, IntoElement, ParentElement, RenderOnce, SharedString, Styled, Window,
...
crate::impl_variant_size_radius_via_methods!(Stepper, variant, size, radius);
...
impl RenderOnce for Stepper {
    fn render(mut self, window: &mut gpui::Window, _cx: &mut gpui::App) -> impl IntoElement {
        ...
        if let Some(active_step) = self.steps.get_mut(active)
            && let Some(content) = active_step.content.take()
        {
        ...
        let step_nodes = self
            ...
            .map(|(index, step)| {
                ...
                let step_id = self.id.slot_index("step", index.to_string());
                let item = match self.orientation {
                ...
                let mut item =
                    apply_radius(&self.theme, item.into_div(), self.radius).id(step_id.clone());
                ...
                if step.disabled {
                    ...
                    item = bind_press_adapter(
                        item,
                        PressAdapter::new(step_id).on_activate(Some(activate_handler)),
                    );
                }
...
crate::impl_disableable!(StepperStep, |this, value| this.disabled = value);
