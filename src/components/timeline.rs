...
use gpui::{
    AnyElement, Bounds, IntoElement, ParentElement, RenderOnce, SharedString, Styled, canvas, div,
    fill, point, px, size,
};
...
impl Timeline {
    ...
    fn active_bullet_bg(&self) -> gpui::Hsla {
        let base = resolve_hsla(&self.theme, self.theme.components.timeline.bullet_active_bg);
        match self.variant {
...
crate::impl_variant_size_radius_via_methods!(Timeline, variant, size, radius);
