...
use gpui::{ClickEvent, FontWeight, Hsla, Pixels, Styled, Window, px};
...
pub trait RadiusTarget: Sized {
    fn with_radius_px(self, radius: Pixels) -> Self;
}

impl<T: Styled> RadiusTarget for T {
    fn with_radius_px(self, radius: Pixels) -> Self {
        self.rounded(radius)
    }
}

impl RadiusTarget for super::Stack {
    fn with_radius_px(self, radius: Pixels) -> Self {
        self.rounded(radius)
    }
}

pub fn apply_radius<T: RadiusTarget>(theme: &Theme, node: T, radius: Radius) -> T {
    node.with_radius_px(resolve_radius(theme, SemanticRadiusToken::from(radius)))
}
