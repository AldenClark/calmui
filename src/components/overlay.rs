...
use gpui::{
    AnyElement, ClickEvent, Hsla, IntoElement, ParentElement, RenderOnce, Styled, Window, div,
};
...
impl RenderOnce for Overlay {
    fn render(mut self, _window: &mut gpui::Window, cx: &mut gpui::App) -> impl IntoElement {
        ...
        let token = self
            .color
            .unwrap_or_else(|| self.theme.components.overlay.bg);
...
crate::impl_visible!(Overlay, |this, value| this.visible = value);
