...
use gpui::{
    AnyElement, ElementId, FontStyle, FontWeight, IntoElement, ParentElement, Refineable,
    RenderOnce, SharedString, StrikethroughStyle, Styled, StyledText, TextRun, UnderlineStyle,
    Window, div, font, img, px,
};
...
fn list_item_prefix(order: usize, ordered: bool, depth: usize) -> String {
    const NUMBERED_PREFIXES_UPPER: &str = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
    const NUMBERED_PREFIXES_LOWER: &str = "abcdefghijklmnopqrstuvwxyz";
    const BULLETS: [&str; 5] = ["•", "◦", "▪", "‣", "⁃"];
    ...
    if ordered {
        ...
        return match depth {
            ...
            1 => format!(
                "{}.",
                NUMBERED_PREFIXES_UPPER
                    .chars()
                    .nth(index % NUMBERED_PREFIXES_UPPER.len())
                    .unwrap_or('A')
            ),
            _ => format!(
                "{}.",
                NUMBERED_PREFIXES_LOWER
                    .chars()
                    .nth(index % NUMBERED_PREFIXES_LOWER.len())
                    .unwrap_or('a')
            ),
        };
    }

    BULLETS[depth.min(BULLETS.len() - 1)].to_string()
}
...
fn finish_paragraph(inlines: Vec<InlineNode>) -> Option<MarkdownBlock> {
    ...
    if trimmed.len() == 1
        && let InlineNode::Image { src, title, alt } = &trimmed[0]
    {
    ...
    for segment in segments {
        ...
        if segment.style.code || segment.style.math {
            ...
            run.underline = Some(UnderlineStyle {
                thickness: px(1.0),
                color: Some(palette.inline_code_border.opacity(0.7)),
                wavy: false,
            });
        }
        ...
        if segment.style.kbd {
            ...
            run.underline = Some(UnderlineStyle {
                thickness: px(1.0),
                color: Some(palette.kbd_border.opacity(0.75)),
                wavy: false,
            });
        }
        ...
        if let Some(link) = &segment.link {
            ...
            run.background_color = Some(palette.link.opacity(0.08));
            run.underline = Some(UnderlineStyle {
                thickness: px(1.0),
                color: Some(palette.link.opacity(0.55)),
                wavy: false,
            });
    ...
    fn render(mut self, window: &mut gpui::Window, _cx: &mut gpui::App) -> impl IntoElement {
        ...
        fn heading_style(
            ...
            match level.clamp(1, 6) {
                1 => (px(32.0), px(40.0), FontWeight::BOLD, px(4.0), px(6.0)),
                2 => (px(24.0), px(32.0), FontWeight::SEMIBOLD, px(4.0), px(5.0)),
                3 => (px(20.0), px(28.0), FontWeight::SEMIBOLD, px(3.0), px(4.0)),
                4 => (px(16.0), px(24.0), FontWeight::MEDIUM, px(2.0), px(3.0)),
                5 => (px(14.0), px(22.0), FontWeight::MEDIUM, px(2.0), px(3.0)),
                _ => (px(13.0), px(20.0), FontWeight::MEDIUM, px(2.0), px(3.0)),
            }
        }
        ...
        fn render_blocks(
            ...
            paragraph_muted: gpui::Hsla,
            _heading2_border: gpui::Hsla,
            quote_bg: gpui::Hsla,
            ...
            for (index, block) in blocks.iter().enumerate() {
                ...
                let element = match block {
                    MarkdownBlock::Heading { level, inlines } => {
                        let (size, line_height, weight, padding_top, padding_bottom) =
                            heading_style(*level);
                        let heading_color = if *level == 6 {
                            paragraph_muted
                        } else {
                            heading_color
                        };
                        let mut top_padding = padding_top;
                        if *level == 2 {
                            top_padding =
                                px(f32::from(top_padding) + f32::from(tokens.heading2_padding_top));
                        }
                    ...
                    MarkdownBlock::Paragraph(inlines) => node()
                    ...
                    MarkdownBlock::Quote(children) => {
                        let quote_rule_width = super::utils::quantized_stroke_px(window, 3.0);
                        let quote_content = node()
                            ...
                            .line_height(tokens.quote_line_height)
                            .text_color(quote_fg)
                            ...
                            .children([node()
                                ...
                                .children([
                                    node()
                                        ...
                                        .bg(quote_border)
                                        .into_any_element(),
                                    quote_content.into_any_element(),
                                ])])
                            .into_any_element()
                    }
                    MarkdownBlock::Code { lang, code } => {
                        ...
                        content
                            ...
                            .w_full()
                            .px(tokens.code_padding)
                            .py(tokens.code_padding)
                            .rounded(tokens.code_radius)
                            .bg(code_bg)
                            .border(super::utils::quantized_stroke_px(window, 1.0))
                            .border_color(code_border.opacity(0.85))
                            .into_any_element()
                    }
                    MarkdownBlock::List {
                        ordered,
                        ...
                        for (item_index, item) in items.iter().enumerate() {
                            let marker_text = list_item_prefix(
                                (*start as usize).saturating_add(item_index),
                                *ordered,
                                list_depth,
                            );
                    ...
                    MarkdownBlock::Table {
                        ...
                        let stroke = super::utils::quantized_stroke_px(window, 1.0);
                        ...
                        if !header.is_empty() {
                            ...
                            for (cell_index, cell) in header.iter().enumerate() {
                                ...
                                if cell_index > 0 {
                                    header_row =
                                        header_row.children([node().w(stroke).bg(table_border)]);
                                }
                                header_row = header_row.children([cell_node.child(
                            ...
                            table = table.children([header_row]);
                        }

                        for (row_index, row) in rows.iter().enumerate() {
                            if row_index > 0 || !header.is_empty() {
                                table =
                                    table.children([node().w_full().h(stroke).bg(table_border)]);
                            }
                            ...
                            for (cell_index, cell) in row.iter().enumerate() {
                                ...
                                if cell_index > 0 {
                                    row_node =
                                        row_node.children([node().w(stroke).bg(table_border)]);
                                }
                                row_node = row_node.children([cell_node.child(
                                    interactive_text_from_inlines(
                                        markdown_id.slot_index(
                                            "table-inline",
                                            format!("{id_key}-{row_index}-{cell_index}"),
                    ...
                    MarkdownBlock::Image { src, title, alt } => {
                        ...
                        let mut image_block =
                            Stack::vertical().gap(tokens.image_gap).children([node()
                                .w_full()
                                .overflow_hidden()
                                ...
                                .child(img(src.clone()).w_full())]);

                        if !caption.trim().is_empty() {
                            image_block = image_block.children([node()
        ...
        root = root.children([render_blocks(
            &self.id,
            blocks.as_slice(),
            window,
            tokens,
        ...
        root.style_mut().refine(&self.style);
        root
    }
}
